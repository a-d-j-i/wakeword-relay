...
class StreamingModel {
 public:
  ...
  // Returns the current sliding-window average probability (0.0–1.0), independent of the cutoff.
  // Safe to call from any task — reads the ring buffer without modifying it.
  float get_sliding_average() const {
    if (this->sliding_window_size_ == 0 || this->recent_streaming_probabilities_.empty())
      return 0.0f;
    uint32_t sum = 0;
    for (uint8_t p : this->recent_streaming_probabilities_)
      sum += p;
    return static_cast<float>(sum / this->sliding_window_size_) / 255.0f;
  }

  // Returns the highest sliding-average seen since the last call, then resets to 0.
  // Use this for polling displays: the inference task runs every ~30ms but the display
  // polls every 100ms, so the instantaneous sliding average is almost always 0 by poll time.
  // Written by Core 1 (inference), read+reset by Core 0 (display). Float writes are atomic
  // on ESP32 — the only race is a missed reset, which is acceptable for a game display.
  float get_peak_and_reset() {
    float p = this->peak_probability_;
    this->peak_probability_ = 0.0f;
    return p;
  }
 ...
 protected:
  ...
  volatile float peak_probability_{0.0f};  // peak since last get_peak_and_reset(); cross-core
