...
#endif
{
 public:
  ...
  // PATCHED: also pass the detection probability (0.0-1.0) so automations can tier the response.
  Trigger<std::string, float> *get_wake_word_detected_trigger() { return &this->wake_word_detected_trigger_; }
  ...
  // Returns the highest sliding-average seen by any enabled model since the last call,
  // then resets each model's peak to 0. This avoids the polling race: the inference task
  // runs every ~30ms but the display polls every 100ms, so get_sliding_average() is almost
  // always 0 by poll time. get_peak_and_reset() captures the true per-interval peak.
  float get_max_probability() const {
    float best = 0.0f;
    for (auto *m : this->wake_word_models_) {  // non-const: calls get_peak_and_reset()
      if (m->is_enabled()) {
        float p = m->get_peak_and_reset();
        if (p > best) best = p;
      }
    }
    return best;
 ...
 protected:
  ...
  Trigger<std::string, float> wake_word_detected_trigger_;  // PATCHED: 2nd arg = probability
